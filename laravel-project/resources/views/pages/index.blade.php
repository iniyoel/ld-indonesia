...
.tutor-photo svg{ width: 100%; height: 100%; }
            ...
            <li class="tutor-card" role="listitem">
              <div class="tutor-photo" style="background:{{ $cardColor }};">
                @if($tutor->profile_photo_path)
                  <img src="{{ asset('storage/' . $tutor->profile_photo_path) }}" alt="{{ $tutor->name }}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">
                @else
                  <svg viewBox="0 0 100 100" aria-hidden="true"><circle cx="50" cy="38" r="18" fill="{{ $iconColor }}"/><path d="M15 92c0-19 15-34 35-34s35 15 35 34" fill="{{ $iconColor }}"/></svg>
                @endif
              </div>
              <div class="tutor-info">
                <h3>{{ $tutor->name }}</h3>
                <span class="tutor-role">Tutor aktif</span>
                <p>Berpengalaman membantu siswa belajar bahasa Jerman dengan pendekatan yang terstruktur dan personal.</p>
              </div>
            </li>
          @endforeach
        </ul>
