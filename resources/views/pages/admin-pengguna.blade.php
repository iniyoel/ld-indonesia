...
.modal-field label{ display: block; font-weight: 700; font-size: 0.94rem; color: var(--navy); margin-bottom: 8px; }
...
.modal-input-group input{
  width: 100%; font: inherit; font-size: 0.92rem; padding: 12px 16px 12px 44px;
...
.modal-select-wrap select{
  appearance: none; -webkit-appearance: none; width: 100%; font: inherit; font-size: 0.92rem;
  padding: 12px 42px 12px 16px; border: 1.5px solid var(--gray-200); border-radius: var(--radius-sm);
...
.modal-hint{
  ...
  border-radius: var(--radius-sm); padding: 12px 14px; font-size: 0.82rem; color: var(--gray-600); line-height: 1.6; margin-bottom: 20px;
}
.modal-hint svg{ width: 16px; height: 16px; color: var(--pink-dark); flex-shrink: 0; margin-top: 1px; }
...
.readonly-input-plain{
  width: 100%; font: inherit; font-size: 0.92rem; padding: 12px 16px;
  border: 1.5px solid var(--gray-200); border-radius: var(--radius-sm);
}
.modal-field-row{ display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 18px; }
...
.confirm-sub{ font-size: 0.88rem; color: var(--gray-500); margin-bottom: 24px; }
.confirm-actions{ display: flex; justify-content: center; gap: 12px; }
.btn-confirm-cancel{
  padding: 10px 24px; border-radius: var(--radius-pill); border: 1.5px solid var(--pink-light);
  color: var(--pink-dark); font-weight: 700; font-size: 0.9rem; background: var(--white);
}
.btn-confirm-cancel:hover{ background: var(--pink-pale); }
.btn-confirm-delete{
  padding: 10px 24px; border-radius: var(--radius-pill); border: 1.5px solid transparent;
  color: var(--pink-dark); font-weight: 700; font-size: 0.9rem; background: var(--pink-pale);
}
.btn-confirm-delete:hover{ background: var(--pink-light); }
.btn-confirm-delete:disabled{ opacity: 0.6; cursor: not-allowed; }

.modal-actions{ display: flex; justify-content: flex-end; gap: 10px; }
.btn-cancel{ padding: 12px 20px; border-radius: var(--radius-pill); font-weight: 700; font-size: 0.92rem; color: var(--navy-soft); }
.btn-cancel:hover{ background: var(--gray-100); }
.btn-save-user{
  display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 12px 26px; border-radius: var(--radius-pill);
  background: linear-gradient(135deg, var(--pink) 0%, var(--pink-dark) 100%); color: var(--white);
  font-weight: 700; font-size: 0.92rem; box-shadow: 0 12px 28px rgba(236,78,140,0.22);
...
.crop-modal-head h3{ font-size: 1.1rem; }
.crop-modal-close{
    width: 36px;
    height: 36px;
    border-radius: 50%;
...
.crop-modal-close:hover{ background: var(--gray-100); }
.crop-editor{ padding: 22px; }
.crop-stage{
    position: relative;
    width: 100%;
    max-width: 560px;
    ...
    <main class="page-content" id="mainContent">
      ...
      >
          ...
          <div class="select-field">
              <label for="statusFilter" class="sr-only" hidden>
                  Filter status
              </label>
              ...
              >
                  ...
                  <option value="non_aktif"
                      {{ request('status') === 'non_aktif' ? 'selected' : '' }}>
                      Non-Aktif
                  </option>
              </select>
          ...
          <div class="select-field">
              <label for="roleFilter" class="sr-only" hidden>
                  Filter kategori pengguna
              </label>
      ...
      <section class="panel" aria-label="Daftar pengguna">
        ...
        <div
            class="table-footer"
            id="tableFooter"
            @if(isset($users) && $users->total() === 0) hidden @endif
        >
            <div class="rows-per-page">
                ...
                >
                    <option value="15"
                        {{ request('per_page', 15) == 15 ? 'selected' : '' }}>
                        15
                    </option>
                    <option value="25"
                        {{ request('per_page', 15) == 25 ? 'selected' : '' }}>
                        25
                    </option>
                    <option value="50"
                        {{ request('per_page', 15) == 50 ? 'selected' : '' }}>
                        50
                    </option>
                </select>

                <span class="results-count">
                    {{ isset($users) ? ($users->firstItem() ?? 0) : 0 }}
                    –
                    {{ isset($users) ? ($users->lastItem() ?? 0) : 0 }}
                    dari
                    {{ isset($users) ? $users->total() : 0 }} pengguna
                </span>
            </div>
  ...
  <div class="modal-card">
    ...
    <form class="modal-body" id="addUserForm" novalidate>
      ...
      <div class="modal-field" id="newUserPhotoField" style="margin-bottom:16px;">
        ...
        <p class="modal-field-help" id="newUserPhotoHelp" style="margin-top:8px;font-size:0.86rem;color:var(--gray-600);">Opsional untuk siswa/admin. Wajib untuk tutor agar foto tampil di landing page.</p>
      ...
      <div class="modal-field" id="newUserDescriptionField" style="margin-bottom:16px; display:none;">
          ...
          ></textarea>
          <p class="modal-field-help" style="margin-top:8px;font-size:0.86rem;color:var(--gray-600);">
      ...
      <div class="modal-field" style="margin-bottom:16px;">
        <label class="checkbox-label" style="display:flex;align-items:center;gap:10px;cursor:pointer;">
          ...
          <span style="font-size: 0.92rem; font-weight: 600; color: var(--navy);">Buat password otomatis untuk akun ini</span>
        </label>
        <p class="modal-field-help" style="margin-top:8px;font-size:0.86rem;color:var(--gray-600);">Jika opsi ini aktif, akun dibuat dengan password otomatis. Pengguna dapat mengatur ulang password sendiri melalui halaman lupa password.</p>
      </div>
...
@endif
  ...
  function render(){
    ...
    if (tableFooter) tableFooter.hidden = isEmpty;
  ...
  function resetForm(){
      ...
      setFieldError('newUserDescription', 'newUserDescriptionError', false);

      if (photoError) photoError.classList.remove('show');
  ...
  function toggleLevelField(){
    ...
    if (levelField) levelField.style.display = showLevel ? 'block' : 'none';
    if (!showLevel && levelSelect){
      levelSelect.value = '';
      setFieldError('newUserLevel', 'newUserLevelError', false);
    }
  }
  function togglePhotoRequirement(){
      ...
      if (descriptionField) descriptionField.style.display = isTutor ? 'block' : 'none';

      if (!isTutor && descriptionInput) {
  ...
  function updateCropImage(){
      if (!cropImage.naturalWidth || !cropImage.naturalHeight || !cropStage) return;
  ...
  if (cropStage) {
    ...
    cropStage.addEventListener('pointermove', function(e){
        ...
        cropState.x = cropState.startImageX + (e.clientX - cropState.startX);
        cropState.y = cropState.startImageY + (e.clientY - cropState.startY);
        updateCropImage();
    });
  ...
  function createCroppedPhoto(callback){
      ...
      var imageX = (stageWidth - renderedWidth) / 2 + cropState.x;
      var imageY = (stageHeight - renderedHeight) / 2 + cropState.y;
      ...
      sourceX = Math.max(0, Math.min(sourceX, cropImage.naturalWidth - sourceWidth));
      sourceY = Math.max(0, Math.min(sourceY, cropImage.naturalHeight - sourceHeight));
      sourceWidth = Math.min(sourceWidth, cropImage.naturalWidth - sourceX);
      sourceHeight = Math.min(sourceHeight, cropImage.naturalHeight - sourceY);
  ...
  if (addUserBtn) addUserBtn.addEventListener('click', function(e){ e.preventDefault(); openModal(); });
  ...
  function openViewModal(user){
    ...
    if (viewUserLevelField) viewUserLevelField.style.display = isSiswa ? 'block' : 'none';
  ...
  document.addEventListener('keydown', function(e){
    ...
    if (viewModal && viewModal.classList.contains('show')) closeViewModal();
  ...
  if (addUserForm) {
    addUserForm.addEventListener('submit', function(e){
      ...
      var descriptionOk = !isTutor || (descriptionInput && descriptionInput.value.trim().length > 0);
      ...
      if (!nameOk || !emailOk || !roleOk || !levelOk || !descriptionOk) return;
      ...
      var description = role === 'Tutor' ? (descriptionInput ? descriptionInput.value.trim() : '') : '';
      ...
      if (role === 'Tutor') {
          if (!croppedPhotoBlob) {
              if (photoError) photoError.classList.add('show');
              saveBtn.disabled = false;
              saveBtn.classList.remove('is-loading');
              saveBtnLabel.textContent = 'Simpan';
              return;
          }
          formData.append('photo', croppedPhotoBlob, 'tutor-profile.jpg');
      }
  ...
  function toggleEditLevelField(){
    ...
    if (editLevelField) editLevelField.style.display = showLevel ? 'block' : 'none';
    if (!showLevel) setFieldError('editUserLevel', 'editUserLevelError', false);
  }
  if (editRoleSelect) editRoleSelect.addEventListener('change', toggleEditLevelField);

  function setEditStatusToggle(isAktif){
    if (!editStatusToggle || !editStatusLabel) return;
