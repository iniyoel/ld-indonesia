...
<title>Detail Pengerjaan — Simulasi Hören — LD Indonesia</title>
<meta name="description" content="Detail hasil pengerjaan simulasi Hören siswa — LD Indonesia.">
...
.audio-player{ display:flex; align-items:center; gap:16px; background:var(--pink-pale); border:1px solid var(--pink-light); border-radius:var(--radius-md); padding:16px 20px; margin-bottom:24px; }
...
.image-options{ display:grid; grid-template-columns:repeat(2, 1fr); gap:16px; margin-bottom:6px; }
.image-option{ border:1.5px solid var(--gray-200); border-radius:var(--radius-md); padding:18px; display:flex; flex-direction:column; align-items:center; gap:10px; position:relative; }
.image-thumb{ width:64px; height:64px; border-radius:12px; background:var(--white); display:flex; align-items:center; justify-content:center; color:var(--navy-soft); }
.image-thumb svg{ width:32px; height:32px; }
.image-option-letter{ font-weight:800; font-size:0.86rem; color:var(--navy); }
.image-option-tag{ position:absolute; top:10px; right:10px; font-size:0.68rem; font-weight:800; padding:3px 9px; border-radius:var(--radius-pill); }

.image-option.is-correct{ border-color:var(--green-border); background:var(--green-bg); }
.image-option.is-correct .image-thumb{ color:var(--green); }
.image-option.is-correct .image-option-letter{ color:var(--green); }
.image-option.is-correct .image-option-tag{ background:var(--green); color:var(--white); }

.image-option.is-wrong-selected{ border-color:var(--red-border); background:var(--red-bg); }
.image-option.is-wrong-selected .image-thumb{ color:var(--red); }
.image-option.is-wrong-selected .image-option-letter{ color:var(--red); }
.image-option.is-wrong-selected .image-option-tag{ background:var(--red); color:var(--white); }

.info-note{ display:flex; align-items:center; gap:8px; font-size:0.82rem; color:var(--gray-500); margin-bottom:6px; }
.info-note svg{ width:15px; height:15px; flex-shrink:0; }
    ...
    <main class="page-content" id="mainContent">
      ...
      <section class="result-panel" aria-labelledby="resultTitle">
        <div class="result-head">
          <h1 id="resultTitle">Modal Verben</h1>
          <span class="score-chip" id="scoreChip">Benar: 12/15</span>
        </div>
        <p class="result-desc">Simulasi Hören hanya menampilkan jawaban yang benar untuk tiap soal — tanpa deskripsi, karena latihan ini berfokus pada kemampuan mendengar.</p>
        ...
        <div class="quiz-layout">
          <section class="quiz-card" aria-label="Detail jawaban soal">
            ...
            <div class="audio-player">
              <button class="play-btn" id="playBtn" type="button" aria-label="Putar ulang audio">
              ...
              <span class="audio-hint">Dengarkan ulang audio soal ini</span>
            </div>
            ...
            <p class="info-note">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="M12 8v5M12 16h.01"/></svg>
              Simulasi Hören tidak menampilkan deskripsi jawaban.
            </p>
...
(function(){
  ...
  /* ==================================================================
     ...
     - QUESTIONS di bawah ini masih data contoh statis. Saat backend
       siap, ganti dengan fetch('/api/riwayat/{attemptId}') termasuk
       berkas audio asli tiap soal.
     - Sesuai ketentuan, halaman ini SENGAJA tidak menampilkan kotak
       deskripsi/penjelasan jawaban — hanya menandai opsi gambar yang
       benar (hijau) dan pilihan siswa yang salah (merah, jika ada).
  ================================================================== */
  var ICONS = {
  ...
  var LETTERS = ['A', 'B', 'C', 'D'];
  var QUESTIONS = [
    { text: 'Wo trifft sich die Familie am Wochenende?', options: ['haus', 'auto', 'buch', 'kaffee'], correct: 0, selected: 0 },
    { text: 'Womit fährt Anna zur Arbeit?', options: ['zug', 'auto', 'haus', 'schule'], correct: 0, selected: 0 },
    { text: 'Was macht Peter jeden Abend vor dem Schlafen?', options: ['buch', 'kaffee', 'uhr', 'regen'], correct: 0, selected: 1 },
    { text: 'Wie ist das Wetter heute laut dem Sprecher?', options: ['regen', 'haus', 'auto', 'buch'], correct: 0, selected: 0 },
    { text: 'Um wie viel Uhr beginnt der Unterricht?', options: ['uhr', 'schule', 'zug', 'kaffee'], correct: 0, selected: 0 },
    { text: 'Wohin geht die Familie am Sonntagmorgen?', options: ['schule', 'haus', 'zug', 'regen'], correct: 1, selected: 1 },
    { text: 'Was trinkt Herr Müller jeden Morgen?', options: ['kaffee', 'buch', 'uhr', 'auto'], correct: 0, selected: 0 },
    { text: 'Womit kommt Lisa zur Schule?', options: ['auto', 'zug', 'haus', 'schule'], correct: 1, selected: 1 },
    { text: 'Was macht Familie Schmidt am Nachmittag?', options: ['buch', 'regen', 'kaffee', 'uhr'], correct: 0, selected: 2 },
    { text: 'Wo wohnt die Familie jetzt?', options: ['haus', 'schule', 'zug', 'auto'], correct: 0, selected: 0 },
    { text: 'Was braucht Tom für die Reise?', options: ['auto', 'buch', 'zug', 'uhr'], correct: 2, selected: 2 },
    { text: 'Wann klingelt der Wecker?', options: ['uhr', 'kaffee', 'regen', 'haus'], correct: 0, selected: 0 },
    { text: 'Wie ist das Wetter am Montag?', options: ['regen', 'zug', 'schule', 'buch'], correct: 0, selected: 0 },
    { text: 'Wo lernt Sara Deutsch?', options: ['schule', 'auto', 'kaffee', 'haus'], correct: 0, selected: 0 },
    { text: 'Womit reist die Familie nach Berlin?', options: ['zug', 'buch', 'uhr', 'regen'], correct: 0, selected: 0 }
  ];

  var totalCorrect = QUESTIONS.filter(function(q){ return q.selected === q.correct; }).length;
  document.getElementById('scoreChip').textContent = 'Benar: ' + totalCorrect + '/' + QUESTIONS.length;
  ...
  function renderQuestion(){
    ...
    q.options.forEach(function(iconKey, i){
      ...
      div.className = 'image-option' + (isCorrect ? ' is-correct' : '') + (isWrongSelected ? ' is-wrong-selected' : '');
      var tag = isCorrect ? '<span class="image-option-tag">Benar</span>' : (isWrongSelected ? '<span class="image-option-tag">Jawabanmu</span>' : '');
      div.innerHTML =
        tag +
  ...
  var isPlaying = false, audioInterval = null, audioElapsed = 0;
  var AUDIO_DURATION = 32;
  function resetAudioPreview(){
    clearInterval(audioInterval); isPlaying = false; audioElapsed = 0;
    audioProgress.style.width = '0%'; audioCurrent.textContent = '0:00';
    ...
    if (isPlaying){
      audioInterval = setInterval(function(){
        ...
        if (audioElapsed >= AUDIO_DURATION){
          audioElapsed = AUDIO_DURATION; isPlaying = false;
          playIcon.innerHTML = '<path d="M8 5v14l11-7z"/>';
          clearInterval(audioInterval);
        }
        audioProgress.style.width = (audioElapsed / AUDIO_DURATION * 100) + '%';
        var m = Math.floor(audioElapsed / 60), s = audioElapsed % 60;
