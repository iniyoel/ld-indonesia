...
.back-link{ display:inline-flex; align-items:center; justify-content:center; gap:8px; padding:10px 18px; border-radius:var(--radius-pill); border:1.5px solid var(--pink-light); color:var(--pink-dark); font-weight:700; font-size:0.9rem; margin-bottom:18px; }
...
.result-head h1{ font-size:1.5rem; word-break:break-word; }
...
.result-desc{ color:var(--gray-500); font-size:0.9rem; margin-bottom:22px; word-break:break-word; }

.quiz-layout{ display:flex; gap:24px; align-items:flex-start; width:100%; min-width:0; }
.quiz-card{ 
  flex: 1 1 auto; 
  min-width: 0; 
  background: var(--white); 
...
.quiz-question{ font-size:1.12rem; color:var(--gray-800); font-weight:500; margin-bottom:22px; word-break:break-word; }

.quiz-options{ display:flex; flex-direction:column; gap:14px; margin-bottom:22px; width:100%; min-width:0; }
.quiz-option{ 
  display: flex; 
  align-items: center; 
  gap: 16px; 
...
.quiz-option-text{ font-size:0.98rem; color:var(--gray-800); flex:1 1 200px; min-width:0; word-break:break-word; }
...
.explanation-box{
  ...
  padding:16px 20px; display:flex; gap:12px; width:100%; box-sizing:border-box;
}
...
.explanation-text{ font-size:0.9rem; color:var(--gray-600); line-height:1.6; word-break:break-word; }

.quiz-actions{ margin-top:auto; padding-top:24px; display:flex; align-items:center; justify-content:flex-end; gap:12px; flex-wrap:wrap; width:100%; }
.nav-btn{ display:inline-flex; align-items:center; justify-content:center; gap:8px; padding:12px 26px; border-radius:var(--radius-pill); font-weight:700; font-size:0.94rem; }
...
.soal-grid{ display:grid; grid-template-columns:repeat(3, 1fr); gap:10px; margin-bottom:22px; width:100%; }
.soal-btn{ aspect-ratio:1; border-radius:10px; border:1.5px solid var(--gray-200); background:var(--white); color:var(--navy); font-weight:700; font-size:0.92rem; display:flex; align-items:center; justify-content:center; cursor:pointer; }
...
@media (max-width:980px){
  ...
  .quiz-layout{ flex-direction:column; gap:20px; }
  .quiz-sidebar{ flex:1 1 auto; width:100%; position:static; }
  .quiz-card{ padding:22px 18px; min-height:auto; }
...
<script>
    ...
    var legendMultipleChoice = document.getElementById('legendMultipleChoice');
    var legendEssay = document.getElementById('legendEssay');
...
(function(){
    ...
    if (!QUESTIONS.length) {
        ...
        quizQuestion.textContent = 'Belum ada soal untuk modul ini.';
    ...
    function renderQuestion(){
        ...
        if (
            ...
            audioLabel.className = 'question-audio-label';
            audioLabel.textContent = 'Audio Soal';

            var audio = document.createElement('audio');
            audio.className = 'question-audio';
            audio.controls = true;
            audio.preload = 'metadata';
            audio.src = '{{ asset('storage') }}/' + q.file_path;
        ...
        if (
            ...
            var essayBox = document.createElement('div');
            essayBox.className = 'essay-answer-box';

            var essayLabel = document.createElement('div');
            essayLabel.className = 'essay-answer-label';
            essayLabel.textContent = 'Jawabanmu';

            var essay = document.createElement('div');
            essay.className = 'essay-answer';
            ...
            if (
                ...
                essay.textContent = q.answer_text;
            } else {
                essay.textContent = 'Siswa belum memberikan jawaban.';
                essay.classList.add('essay-empty');
            }
            ...
            q.options.forEach(function(option, i){
                ...
                var isSelected = option.is_selected === true || option.is_selected === 1 || option.is_selected === "1";
                var isWrongSelected = isSelected && !isCorrect;

                var row = document.createElement('div');
                var hasImage = option.file_path && option.file_type && String(option.file_type).startsWith('image/');
                ...
                if (isSelected && isCorrect) {
                    tag = '<span class="quiz-option-tag">Jawabanmu • Jawaban Benar</span>';
                } else if (isSelected && isWrongSelected) {
                    tag = '<span class="quiz-option-tag">Jawabanmu</span>';
                } else if (isCorrect) {
                    tag = '<span class="quiz-option-tag">Jawaban Benar</span>';
                }

                var letter = '<span class="quiz-option-letter">' + LETTERS[i] + '</span>';
                var content = '';

                if (hasImage) {
                    content +=
                        '<span class="quiz-option-media">' +
                            '<img class="quiz-option-image" src="{{ asset('storage') }}/' + escapeHtml(option.file_path) + '" alt="Gambar pilihan ' + LETTERS[i] + '">' +
                        '</span>';
                } else {
                    content += '<span class="quiz-option-text">' + escapeHtml(option.text || '') + '</span>';
                }

                row.innerHTML = letter + content + tag;
                quizOptions.appendChild(row);
            });

            explanationText.textContent = q.explanation || 'Tidak ada penjelasan untuk soal ini.';
        }
        ...
        nextBtnLabel.textContent = isLast ? 'Kembali ke Detail Siswa' : 'Selanjutnya';
    ...
    function renderGrid(){
        ...
        QUESTIONS.forEach(function(q, i){
            ...
            if (q.is_answered) {
                if (MODULE_CATEGORY === 'simulasi_schreiben' || q.type === 'paragraf') {
    ...
    if (nextBtn) {
        nextBtn.addEventListener('click', function(){
            ...
            if (isLast) {
                window.location.href = "{{ Auth::user()->role === 'tutor' ? route('tutor.siswa.detail', $student->id) : route('admin.siswa.detail', $student->id) }}";
            } else {
                state.current++;
                renderQuestion();
                renderGrid();
