...
:root{
  ...
  --font-body:'Inter',sans-serif;--radius-sm:10px;--radius-md:16px;--radius-lg:20px;
...
.sidebar{
  width:var(--sidebar-w);
  flex-shrink:0;
  background:linear-gradient(180deg,var(--pink-pale),#FDF1F6);
...
.topbar{
  ...
  gap:16px;
  padding:0 40px;
  background:linear-gradient(115deg,#FCEFD9,#FDE4EE 55%,#FBCFE0);
  position:sticky;
  top:0;
  z-index:40;
}
...
.page-heading{margin-bottom:22px}.page-heading h1{font-size:1.7rem}
...
.select-field select{appearance:none;-webkit-appearance:none;width:100%;font:inherit;font-weight:600;font-size:.92rem;padding:13px 40px 13px 18px;
...
.btn-export{display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:13px 24px;border-radius:var(--radius-pill);
...
thead th{text-align:left;font-size:.85rem;font-weight:700;color:var(--navy);background:var(--pink-light);padding:15px 20px;white-space:nowrap}
thead th:first-child{padding-left:24px}
tbody td{padding:15px 20px;font-size:.92rem;color:var(--gray-800);border-bottom:1px solid var(--gray-100);vertical-align:middle;white-space:nowrap}
tbody td:first-child{padding-left:24px}
tbody tr:last-child td{border-bottom:none}
tbody tr:hover{background:var(--gray-50)}
td.col-name{font-weight:700;color:var(--navy);white-space:normal;min-width:160px;word-break:break-word}
td.col-nilai{font-weight:800;color:var(--navy)}
.activity-badge{display:inline-flex;align-items:center;justify-content:center;min-width:38px;padding:4px 8px;border-radius:999px;background:var(--pink-light);color:var(--pink-dark);font-weight:800;font-size:.82rem}
.activity-muted{color:var(--gray-400);font-weight:600}.activity-date{color:var(--gray-600);font-size:.86rem}
.action-btn{display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:50%;color:var(--pink-dark);transition:background .15s}
.action-btn:hover{background:var(--pink-pale)}.action-btn svg{width:18px;height:18px}
...
.page-btn{min-width:34px;height:34px;padding:0 8px;border-radius:8px;border:1.5px solid var(--gray-200);
...
@media(max-width:980px){
  .sidebar{position:fixed;left:0;top:0;transform:translateX(-100%);transition:transform .22s;box-shadow:var(--shadow-md)}
  ...
  .sidebar-close:hover{background:rgba(30,42,71,.06)}
  .sidebar-brand{justify-content:space-between}
  .menu-toggle{display:flex}
  .topbar{padding:0 20px}
  .page-content{padding:24px 16px 48px}
  .backdrop{display:none;position:fixed;inset:0;background:rgba(30,42,71,.35);z-index:50}
        ...
        <section class="panel" aria-label="Aktivitas seluruh siswa">
          ...
          <div class="table-footer">
            <div class="rows-per-page">
              Menampilkan <strong>{{ $students->firstItem() ?? 0 }}–{{ $students->lastItem() ?? 0 }}</strong> dari <strong>{{ $students->total() ?? $students->count() }}</strong> siswa
            </div>
            ...
            <nav class="pagination" aria-label="Navigasi halaman">
              ...
              @else
                <a class="page-btn" href="{{ $students->previousPageUrl() }}">‹</a>
              @endif
              ...
              @if($students->hasMorePages())
                <a class="page-btn" href="{{ $students->nextPageUrl() }}">›</a>
              @else
                <button class="page-btn" disabled>›</button>
              @endif
            </nav>
...
if(levelFilter) levelFilter.addEventListener('change',filterRows);
...
if(exportBtn){
  exportBtn.addEventListener('click',function(){
    ...
    if(exportLabel) exportLabel.textContent='Ekspor Excel';
  });
}
